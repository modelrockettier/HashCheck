...
static const TCHAR szSaveDefaultName[] = _T("checksums");
...
// Path processing
void HashCalcWalkDirectory( HASHCONTEXT* phcctx, LPTSTR pszPath, UINT cchPath );
__inline BOOL IsSpecialDirectoryName( LPCTSTR pszPath );
__inline BOOL IsDoubleSlashPath( LPCTSTR pszPath );

// Save helpers
__inline void HashCalcSetSavePrefix( HASHCONTEXT* phcctx, LPTSTR pszSave );
...
{
   LPTSTR pszPrev = NULL;
   LPTSTR pszCurrent, pszCurrentEnd;
   UINT cbCurrent, cchCurrent;

   SLReset(phcctx->hListRaw);

   while (pszCurrent = (LPTSTR) SLGetDataAndStepEx(phcctx->hListRaw, &cbCurrent))
   {
      pszCurrentEnd = (LPTSTR) BYTEADD(pszCurrent, cbCurrent);
      cchCurrent = cbCurrent / sizeof(TCHAR) - 1;

      // Get rid of the trailing slash if there is one
      if (cchCurrent && *(pszCurrentEnd - 1) == _T('\\'))
      {
         *(--pszCurrentEnd) = 0;
         --cchCurrent;
      ...
      if (!pszPrev)
      {
         ...
         LPTSTR pszTail = StrRChr(pszCurrent, pszCurrentEnd, _T('\\'));
      ...
      if (cchCurrent && phcctx->hList)
      {
         ...
         {
            HASHITEM* pItem = (HASHITEM*) SLAddItem(phcctx->hList, NULL, sizeof(HASHITEM) + cbCurrent);
      ...
      if (phcctx->parms.status == CANCEL_REQUESTED)
         return;
...
void HashCalcWalkDirectory( HASHCONTEXT* phcctx, LPTSTR pszPath, UINT cchPath )
{
   ...
   LPTSTR pszPathAppend = pszPath + cchPath;
   *pszPathAppend = _T('\\');
   SSCpy2ChW(++pszPathAppend, _T('*'), 0);
   ...
   do
   {
      ...
      UINT cchLeaf = (UINT)wcslen(finddata.cFileName) + 1;
      ...
      {
         SSChainNCpyW(pszPathAppend, finddata.cFileName, cchLeaf);
         ...
         else
         {
            ...
            HASHITEM* pItem = (HASHITEM*) SLAddItem(phcctx->hList, NULL, sizeof(HASHITEM) + cbPathBuffer);
...
BOOL IsSpecialDirectoryName( LPCTSTR pszPath )
{
   // TRUE if name is "." or ".."

   return(
      (*((UPDWORD)pszPath) == WCHARS2DWORD(L'.', 0)) ||
      (*((UPDWORD)pszPath) == WCHARS2DWORD(L'.', L'.') && !pszPath[2])
   );
}

BOOL IsDoubleSlashPath( LPCTSTR pszPath )
{
...
void HashCalcInitSave( HASHCONTEXT* phcctx )
{
   ...
   if (!phcctx->ofn.lStructSize)
   {
      ...
      // Set the initial file name
      {
         ...
         SLReset(phcctx->hListRaw);
         pszOrigPath = (LPTSTR) SLGetDataAndStep(phcctx->hListRaw);
         ...
         if (SLCheck(phcctx->hListRaw))
         {
            ...
            SSChainNCpy2W(
               ...
               szSaveDefaultName, _countof(szSaveDefaultName)
            );
         }
         else
         {
            // Only one item was selected in Explorer (could be a
            // single file or a directory containing multiple files)
            SSChainCpyCatW(pszFile, pszOrigPath, g_szHashExtsTab[phcctx->ofn.nFilterIndex-1]);
         }
      }
   }
   ...
   if ( GetSaveFileName(&phcctx->ofn) &&
        ...
        phcctx->ofn.nFilterIndex <= NUM_HASH_TYPES )
   {
      BOOL bSuccess = FALSE;
      ...
      else
      {
         ...
         LoadString(g_hModThisDll, IDS_HC_SAVE_ERROR, szMessage, _countof(szMessage));
...
BOOL HashCalcWriteResult( HASHCONTEXT* phcctx, HASHITEM* pItem )
{
   ...
   if (!phcctx->szFormat[0])
   {
      ...
      {
         SSChainNCpyW(phcctx->szFormat, _T("%s *%s") _T(CRLF), _countof(_T("%s *%s") _T(CRLF)));
      }
   }
   ...
   if (cchLine > 0)
   {
      ...
      switch (phcctx->opt.dwSaveEncoding)
      {
         ...
         {
            // UTF-8
            cbLine = WStrToUTF8(phcctx->scratch.szW, phcctx->scratch.szA, _countof(phcctx->scratch.szA)) - 1;
         ...
         {
            // ANSI
            cbLine = WStrToAStr(phcctx->scratch.szW, phcctx->scratch.szA, _countof(phcctx->scratch.szA)) - 1;
            pvLine = phcctx->scratch.szA;
            break;
         }
      }
...
void HashCalcSetSavePrefix( HASHCONTEXT* phcctx, LPTSTR pszSave )
{
   ...
   SLReset(phcctx->hListRaw);
   pszOrig = (LPTSTR) SLGetDataAndStep(phcctx->hListRaw);
   ...
   // %TEMP% on a NT 5.x system)
   {
      ...
      LPTSTR pszOrigLong = (LPTSTR)phcctx->scratch.szW;
      LPTSTR pszSaveLong = (LPTSTR)phcctx->scratch.szA;

      // Copy original path to scratch and terminate
      pszTail = SSChainNCpyW(pszOrigLong, pszOrig, phcctx->cchPrefix);
      pszTail[0] = 0;

      // Copy output path to scratch and terminate
      pszTail = SSChainNCpyW(pszSaveLong, pszSave, phcctx->ofn.nFileOffset);
      pszTail[0] = 0;
   ...
   if (pszTail = StrRChr(pszSave, NULL, _T('\\')))
   {
   ...
   if ( pszSave[1] == _T(':') && phcctx->cchPrefix > 2 &&
...
{
   DWORD dwStyle = (DWORD)GetWindowLongPtr(phcctx->parms.hWndPBTotal, GWL_STYLE);

   if (bState)
   {
      dwStyle &= ~PBS_SMOOTH;
      dwStyle |= PBS_MARQUEE;
      phcctx->parms.dwFlags |= HCF_MARQUEE;
   }
   else
   {
      dwStyle |= PBS_SMOOTH;
   ...
   SetWindowLongPtr(phcctx->parms.hWndPBTotal, GWL_STYLE, dwStyle);
   SendMessage(phcctx->parms.hWndPBTotal, PBM_SETMARQUEE, bState, MARQUEE_INTERVAL);

   if (!bState)
      SendMessage(phcctx->parms.hWndPBTotal, PBM_SETRANGE32, 0, phcctx->cTotal);
}
