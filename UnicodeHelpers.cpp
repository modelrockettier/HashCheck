...
PWSTR __fastcall BufferToWStr( PBYTE *ppbData, DWORD cbData )
{
   ...
   if (iUnicodeTests & IS_TEXT_UNICODE)
   {
      ...
      if (iUnicodeTests & IS_TEXT_UNICODE_REVERSE_MASK)
         SwapA16((unsigned short *)pbData, cbData / sizeof(unsigned short));
   ...
   // Step 2: If this is not UTF-16, then check if this is UTF-8 or ANSI,
   // and then convert as appropriate

   if (pszResult = (PWSTR) malloc((cbData + 1) * sizeof(WCHAR)))
   {
      PBYTE pbScratch;
      UINT uCodePage;
      ...
      if (AStrToWStrEx((LPCSTR) pbScratch, pszResult, cbData + 1, uCodePage))
      {
         free(pbData);
